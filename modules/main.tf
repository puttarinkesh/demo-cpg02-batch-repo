...
resource "azurerm_resource_group" "dev_rg" {
  name     = "rg-${var.env}-${var.postfix}-${app}"
...
resource "azurerm_network_interface" "nic" {
  name                = "${var.env}-${var.postfix}-${app}-nic"
  location            = azurerm_resource_group.dev_rg.location
  resource_group_name = azurerm_resource_group.dev_rg.name
  tags                = var.tags

  ip_configuration {
    name                          = "${var.env}-${var.postfix}-${app}-nic-ip"
...
resource "azurerm_windows_virtual_machine" "module_vm" { 
  name                  = "${var.env}-${var.postfix}-${app}-${var.module_vm_name}-vm"
  location              = azurerm_resource_group.dev_rg.location
  resource_group_name   = azurerm_resource_group.dev_rg.name
  ...
  os_disk {
    name                 = "${var.env}-${var.postfix}-${app}-${var.module_vm_name}-os-disk"
